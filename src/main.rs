...
fn main() -> Result<()> {
    ...
    let mut model_host = ModelHost::start();
    ...
    platform_application().run(move |cx: &mut App| {
        let root = cx.new(|cx| {
            WorkspaceView::new(client.clone(), initial_snapshot.clone(), cx.focus_handle())
        });
...
fn parse_startup_options(mut args: impl Iterator<Item = String>) -> Result<StartupOptions> {
    ...
    while let Some(argument) = args.next() {
        if argument == "--control-socket" {
            ...
            println!("water [--control-socket PATH] [--no-initial-terminal] [--empty-workspace]");
            std::process::exit(0);
        } else {
