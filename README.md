...
OSC 8 terminal hyperlinks (for example, `ls --color=auto --hyperlink=auto`) open with the local system's default application when clicked. Enable or disable this in Settings → 超链接 (`terminal.hyperlinks`), or require Cmd/Ctrl-click with `terminal.hyperlink_command_click`. The entire hovered hyperlink range is shown with a wavy underline. Shift-click retains terminal selection behavior, and applications using terminal mouse reporting receive their normal mouse events.
...
server. If the SSH/session transport later drops (for example after changing
networks), the host remains in the sidebar dimmed; choose **Reconnect** from its
context menu to attach it again.
