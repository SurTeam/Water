...
`WaterApplication` owns native-window/menu lifecycle and UI-side connection projections. `Cmd-N` creates another `WorkspaceView`, while each view keeps its own active connection, workspace focus, selection, IME, and layout transients. Each local/SSH connection owns an independent transport and latest revisioned snapshot stream; switching a sidebar server group swaps only the immutable projection used by that window. A GPUI-side per-connection latest-snapshot fan-out updates every live weak view handle; it does not own or mutate any `ApplicationModel`. Window-local actions are installed on the focused `WorkspaceView` root so `Cmd-W`, `Cmd-M`, terminal-tab creation, and split actions receive the correct `Window` and route through the selected connection's `CommandTransport` instead of depending on `App::active_window()`. `Cmd-Q` is both bound to a no-op focused action and consumed by an application keystroke interceptor. `QuitMode::Explicit` keeps the model and PTYs alive when the last window is hidden.

The control socket exposes `ui.keystroke` and `ui.snapshot` as running-application test interfaces. UI requests cross a dedicated channel to the GPUI thread; synthetic keys are dispatched through `Window::dispatch_keystroke`, not translated into direct model mutation. Pane input remains `TerminalCommand::SendText` through the normal dispatcher path, while `waterctl pane content` resolves the pane's terminal snapshot and returns an exact viewport cell range. Remote server groups use the identical Unix-socket protocol through OpenSSH ControlMaster stream-local forwards; SSH setup, platform detection, payload upload, and readiness waits run off the GPUI thread and do not introduce a second mutation path. Disconnect removes the UI projection and forward only; Kill Server explicitly invokes `server.shutdown` before removing them. Version- and protocol-specific remote socket names let compatible clients reattach while incompatible clients start an isolated server rather than replacing a running older one. The macOS bundle is assembled by `scripts/build-macos-app.sh` from the release binary, `assets/macos/Info.plist`, the Water-owned `.icns` resource, and four compressed headless server payloads for Darwin/Linux on arm64/x86_64. The Linux payloads are static musl executables. Only the `uname`-matched payload is streamed to a remote host, atomically installed in that user's cache, and reused by content identity.
...
cursor state, display offset, process state, a monotonic terminal revision, and an eight-row overscan band on either side of the viewport for fractional scrolling.
